...
class Tit {
  constructor(repoPath = ".") {
    this.repoPath = path.join(repoPath, ".tit");
  ...
  async init() {
    ...
    try {
      ...
      console.log("Already initialized tit in this repo");
    }
  }
  ...
  async add(file) {
    ...
    const newFolderHashedObjectPath = path.join(
      this.objectsPath,
      newFolderHashedObjectPathName
    );

    await fs.mkdir(newFolderHashedObjectPath);

    const newFileHashedObjectPathName = fileHash.slice(2);
    const newFileHashedObjectPath = path.join(
      newFolderHashedObjectPath,
      newFileHashedObjectPathName
    );
  ...
  async commit(message) {
    ...
    const commitHashFolderPath = path.join(
      this.objectsPath,
      commitHashFolderPathName
    );

    await fs.mkdir(commitHashFolderPath);

    const commitHashObjectPathName = commitHash.slice(2);
    const commitHashObjectPath = path.join(
      commitHashFolderPath,
      commitHashObjectPathName
    );
